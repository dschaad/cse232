...
namespace custom
{
   ...
   typename BST<T>::iterator BST<T>::erase(iterator& it)
   {
      ...
      else if (node->pLeft && node->pRight)
      {
         ...
         if (succ->pParent->pLeft == succ)
            succ->pParent->pLeft = nullptr;
         if (succ->pParent->pRight == succ)
            succ->pParent->pRight = nullptr;

         // If our successor has a right child, it takes the place of our successor
         if (succ->pRight)
         {
            succ->pRight->pParent = succ->pParent;
            succ->pParent->pLeft = succ->pRight;
         }

         // If node does not have a parent, we need to re-assign the root node
         if (!node->pParent)
            root = succ;

         // Have our successor replace the node to be removed
         succ->pParent = node->pParent;
         succ->pLeft = node->pLeft;
         node->pLeft->pParent = succ;

         // Assuming the node's right child isn't the successor, update the successor's pRight and pRight's parent
         if (!(succ == node->pRight))
         {
            succ->pRight = node->pRight;
            node->pRight->pParent = succ;
         }
         // Otherwise, the successor is the right child, in which case we set the successors pRight to NULL
         else
            succ->pRight = nullptr;

         // Re-assign node->pParent pLeft or pRight to the successor depending on what side the node is on
         if (node->isLeftChild(node))
            node->pParent->pLeft = succ;
         if (node->isRightChild(node))
            node->pParent->pRight = succ;

         delete node;
         // Because we want to return an iterator to the new location of the successor, we need to create a new iterator for itNext
