...
namespace custom
{
   ...
   {
      //// Find the bucket index that would contain the element
      //auto iBucket = bucket(t);

      //// Get a list/local iterator to the element, if it exists
      //auto itList = begin(iBucket);
      //E equals;

      //for (; itList != end(iBucket); itList++)
      //{
      //   if (equals(*itList, t))
      //      break;
      //}
      //using vectorIterator = typename custom::vector<custom::list<T, A>>::iterator;
      //vectorIterator itVector(iBucket, buckets);
